...
script refuses demo mode or server credentials. Its mobile build writes an ignored
receipt containing the clean source SHA and exact APK/AAB hashes. CarsXE credentials
stay on the backend. Record exact source, signatures and served hashes before distribution.
...
assets, verifies the receipt, both signed package versions and public hashes, and automatically advances
